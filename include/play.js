...
module.exports = {
  async play(song, message, client, filters) {
    ...
    if (!song) {
      ...
      const endembed = new MessageEmbed().setColor("#F0EAD6")
        .setAuthor(`Hàng đợi Nhạc đã kết thúc.`, "https://cdn.discordapp.com/attachments/778600026280558617/781024479623118878/ezgif.com-gif-maker_1.gif")
  ...
  try {
      ...
      .setColor("#F0EAD6")
      .setAuthor(`Bắt đầu chơi: ${song.title}`,'https://cdn.discordapp.com/attachments/778600026280558617/781024479623118878/ezgif.com-gif-maker_1.gif')
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        case "⏭":
          ...
          queue.textChannel.send(`${user} ⏩ bỏ qua bài hát`).catch(console.error);
          collector.stop();
          break;
        ...
        case "⏯":
          ...
          if (queue.playing) {
            ...
            queue.textChannel.send(`${user} ⏸ đã tạm dừng nhạc.`).catch(console.error);
          } else {
            queue.playing = !queue.playing;
            queue.connection.dispatcher.resume();
            queue.textChannel.send(`${user} ▶ tiếp tục âm nhạc!`).catch(console.error);
          }
          break;
        ...
        case "🔇":
          ...
          if (queue.volume <= 0) {
            ...
            queue.textChannel.send(`${user} 🔊 bật tiếng nhạc!`).catch(console.error);
          } else {
            queue.volume = 0;
        ...
        case "🔉":
          ...
          queue.textChannel
            .send(`${user} 🔉 giảm âm lượng, âm lượng bây giờ là ${queue.volume}%`)
            .catch(console.error);
          break;
        ...
        case "🔊":
          ...
          queue.connection.dispatcher.setVolumeLogarithmic(queue.volume / 100);
          queue.textChannel
            .send(`${user} 🔊 tăng âm lượng, âm lượng bây giờ là ${queue.volume}%`)
            .catch(console.error);
          break;
        ...
        case "🔁":
          ...
          queue.textChannel.send(`Vòng lặp bây giờ là ${queue.loop ? "**on**" : "**off**"}`).catch(console.error);
          break;
        ...
        case "⏹":
          ...
          if (!canModifyQueue(member)) return;
          queue.songs = [];
          queue.textChannel.send(`${user} ⏹ dừng nhạc!`).catch(console.error);
          try {
            queue.connection.dispatcher.end();
          } catch (error) {
            console.error(error);
        ...
        if (!queue)
          ...
          queue.textChannel
            .send(`${user} 🔀 Đã xáo trộn hàng đợi.`)
            .catch(console.error);
          break;
        ...
        let nowPlaying = new MessageEmbed()
          .setAuthor('♪Đang chơi♪','https://cdn.discordapp.com/attachments/778600026280558617/781024479623118878/ezgif.com-gif-maker_1.gif','http://harmonymusic.tk')
      ...
      if (ms > 0 && ms<10000) {
        nowPlaying.addField("\u200b", "**``[" + createBar((ms == 0 ? seek : ms), seek, 25, "▬", "🔘")[0] + "]``**\n**" + "\n[" + new Date(seek * 1000).toISOString().substr(11, 8) + " / " + (ms == 0 ? " ◉ LIVE" : new Date(ms * 1000).toISOString().substr(11, 8))+ "]**" + "\n" + "\n **Time Remaining:**" + "``" + new Date(left * 1000).toISOString().substr(11, 8) + "``", false );
          ...
          .setAuthor("Đang tìm kiếm...", "https://cdn.discordapp.com/attachments/778600026280558617/781024479623118878/ezgif.com-gif-maker_1.gif").setFooter("Lyrics")
          .setColor("#F0EAD6")
