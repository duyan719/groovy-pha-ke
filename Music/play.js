...
module.exports = {
  ...
  cooldown: 1.5,
  edesc: `Gõ lệnh này để phát nhạc.\nUsage: ${PREFIX}play <TITLE | URL>`,
...
async execute(message, args, client) {
    ...
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Tôi cần quyền để tham gia kênh của bạn!");
    if (!permissions.has("SPEAK"))
      return attentionembed(message, "Tôi cần quyền để nói trong kênh của bạn");
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Đang tìm kiếm 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Đang tìm kiếm 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
    ...
    if (urlValid) {
      try {
        ...
        if (error.statusCode === 403) return attentionembed(message, "Tối đa sử dụng của api Key, vui lòng làm mới!");
        console.error(error);
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .setThumbnail(thumb)
        .setURL(song.url)
        .setDescription(`\`\`\`Đã được thêm vào Hàng đợi.\`\`\``)
        .addField("Thời gian ước tính cho đến khi chơi:", `\`${estimatedtime}\``, true)
        .addField("Vị trí trong hàng đợi ", `**\`${serverQueue.songs.length - 1}\`**`, true)
    ...
    try {
      ...
      return attentionembed(message, `Không thể tham gia kênh: ${error}`);
    }
  }
};
